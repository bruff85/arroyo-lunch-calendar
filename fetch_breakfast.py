#!/usr/bin/env python3
"""
Tustin USD - Arroyo Elementary Breakfast Calendar Generator

Uses the HealthePro API to fetch the monthly breakfast menu and generates
...
ORG_ID          = "547"
SITE_ID         = "4782"
# Last known breakfast menu id (2026-27 school year) — used only as a fallback when
...
ENTREE_CATEGORIES = {"Breakfast Entree", "Entree"}
...
def select_menu(menus, target_month_str):
    ...
    id, prefer whichever menu actually has the target month published:
      ...
      2. a breakfast-named menu that has the target month
    ...
    breakfast = [m for m in with_target if "breakfast" in str(m.get("name", "")).lower()]
    for pool in (known, breakfast, with_target):
        if pool:
            menu = pool[0]
...
def generate_ics(daily_menu, month, year, existing_ics_path=None):
    ...
    for day_date in sorted(daily_menu.keys()):
        ...
        title = " | ".join(items) if items else "Breakfast Menu"
        date_str = day_date.strftime("%Y%m%d")
        uid = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"tustin-breakfast-{date_str}"))
        new_events[date_str] = "\r\n".join([
            "BEGIN:VEVENT",
            f"UID:{uid}",
            f"DTSTAMP:{now}",
            ...
            f"SUMMARY:{title}",
            "DESCRIPTION:Arroyo Elementary School Breakfast Menu - Tustin USD",
            "TRANSP:TRANSPARENT",
            "END:VEVENT",
        ])
    ...
    header = "\r\n".join([
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Tustin Arroyo Elementary Breakfast//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        "X-WR-CALNAME:Arroyo Elementary Breakfast Calendar",
...
def main():
    print("=" * 50)
    print("Arroyo Elementary Breakfast Calendar Generator")
    print("=" * 50)
    ...
    if site_menus and target_month_str not in published:
        ...
        notify_not_found("Arroyo Elementary Breakfast Calendar", target_label)
        return
    ...
    if not daily_menu:
        ...
        notify_found_failure("Arroyo Elementary Breakfast Calendar", target_label, "Menu data returned no entree items.")
        return
    ...
    if old_hash == new_hash:
        ...
        notify_success("Arroyo Elementary Breakfast Calendar", target_label, len(daily_menu))
